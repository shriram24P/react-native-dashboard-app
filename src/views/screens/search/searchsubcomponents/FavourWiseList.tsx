...
const FavourWiseList = ({ navigation }: FavourWiseListScreenProp) => {
  const [searchText, setSearchText] = useState<string>("");
  const [searchResults, setSearchResults] = useState<UserData[]>([]);
  const data: UserData[] = [
    {
      id: 1,
      favour: "not mentioned",
      total: 123,
    },
    {
      id: 2,
  ...
  const renderItem = ({ item }: { item: UserData }) => (
    ...
    >
      ...
      <Text style={[textStyle, { paddingVertical: 10 }]}>{item.favour}</Text>
  ...
  const handleSearch = (text: string) => {
    const filteredResults = data.filter(
      (item) =>
        item.favour.toLowerCase().startsWith(text.toLowerCase()) &&
        item.favour !== text
    );
