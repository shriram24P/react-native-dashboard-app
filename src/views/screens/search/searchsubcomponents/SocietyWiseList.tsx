...
interface SocietyWiseListScreenProp {
  navigation: DrawerNavigationProp<RootDrawerParamList, "SocietyWiseList">;
}
...
const SocietyWiseList = ({ navigation }: SocietyWiseListScreenProp) => {
  ...
  const renderItem = ({ item }: { item: UserData }) => (
    ...
    >
      ...
      <Text style={[textStyle, { paddingVertical: 10 }]}>{item.society}</Text>
  ...
  const handleSearch = (text: string) => {
    const filteredResults = data.filter(
      (item) =>
        item.society.toLowerCase().startsWith(text.toLowerCase()) &&
        item.society !== text
    );
...
export default SocietyWiseList;
