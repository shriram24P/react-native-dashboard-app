...
const PartyWorkerList = ({ navigation }: PartyWorkerListScreenProp) => {
  const [searchText, setSearchText] = useState<string>("");
  const [searchResults, setSearchResults] = useState<UserData[]>([]);
  const data: UserData[] = [
    {
      id: 1,
      partyWorker: "not mentioned",
      total: 123,
    },
    {
      id: 2,
  ...
  const handleSearch = (text: string) => {
    const filteredResults = data.filter(
      (item) =>
        item.partyWorker.toLowerCase().startsWith(text.toLowerCase()) &&
        item.partyWorker !== text
    );
...
export default PartyWorkerList;
